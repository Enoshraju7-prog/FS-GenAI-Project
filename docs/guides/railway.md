...
- A Supabase project from Supabase setup.
- An OpenAI API key.
...
5. In backend **Settings → Deploy**, set the pre-deploy command:
   ...
   ```
6. Deploy the backend, then open **Networking** and generate a public domain.
7. Visit `https://your-backend.up.railway.app/health`. You should see `{"status":"ok"}`.
8. In the same project, click **New → GitHub Repo** and select this repo again.
9. Name the frontend service `document-copilot-frontend`.
...
11. Add frontend **Variables** before deploying:
    ...
    ```
12. Deploy the frontend, then open **Networking** and generate a public domain.
13. Update the backend variable:
    ```
    ALLOWED_ORIGINS=https://your-frontend.up.railway.app
    ```
14. Redeploy the backend.
...
1. Install and authenticate the CLI:
   ...
   ```
2. Ask the agent to confirm Railway access with the MCP `whoami` or `list-projects` tool.
3. Ask the agent to create a private project with MCP:
   ```
...
4. Link your local repo to the project:
   ...
   ```
5. Create the two empty services:
   ```
   railway add --service document-copilot-backend --json
   railway add --service document-copilot-frontend --json
   ```
6. Set backend variables. Use `--stdin` for secret values so they are not printed in shell history:
   ```
   ...
   printf "%s" "$OPENAI_API_KEY" | railway variable set OPENAI_API_KEY \
     ...
     --skip-deploys
   ```
7. Deploy the backend from the `backend/` folder and generate a domain:
   ```
...
8. Run migrations against production:
   ...
   ```
9. Set frontend variables before deploying. `VITE_*` values are baked into the frontend build:
   ```
   railway variable set \
     VITE_API_BASE_URL=https://your-backend.up.railway.app \
     VITE_SUPABASE_URL=https://your-project-ref.supabase.co \
     ...
     --skip-deploys
   ```
10. Deploy the frontend from the `frontend/` folder and generate a domain:
    ```
    ...
    ```
11. Update backend CORS and redeploy:
    ```
    ...
    ```
12. Use MCP `get-status`, `get-logs`, or `railway-agent` to verify both services are `SUCCESS` and diagnose any failures.
