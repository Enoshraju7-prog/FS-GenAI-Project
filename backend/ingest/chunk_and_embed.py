...
def _delete_chunks(session: Session, document_id) -> None:
    # document_chunks.table_id -> document_tables.id is ON DELETE SET NULL and
    # message_citations.chunk_id -> document_chunks.id is ON DELETE CASCADE, so
    # deleting chunks first (before tables) is enough to clean up both.
    session.execute(delete(DocumentChunk).where(DocumentChunk.document_id == document_id))
    session.execute(delete(DocumentTable).where(DocumentTable.document_id == document_id))
