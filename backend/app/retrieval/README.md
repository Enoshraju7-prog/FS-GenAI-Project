...
```mermaid
flowchart TD
    Q[User query + optional SearchFilters] --> E[embed_query]
    Q --> FTSQ[raw query text]
...
1. **Embed the query** — `embeddings.embed_query` embeds the full user question with
   `text-embedding-3-small`. There's no LLM keyword-extraction step (unlike some hybrid-search
   references) — `full_text_search` runs `plainto_tsquery` directly against the raw query text. This
   keeps the pipeline simpler; revisit only if plain-query full-text recall turns out weak in practice.
...
2. **Dual search (parallel)** — `retriever._dual_search` runs semantic and full-text queries
   ...
   against the ingest-generated `search_vector` column and ranks with `ts_rank_cd`. Both return up to
   `candidate_k` hits.
