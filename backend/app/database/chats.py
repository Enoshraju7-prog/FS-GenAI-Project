...
from app.schemas.chat import CitationPart, CitationPayload, ThreadCreated, ThreadSummary, UIMessage
...
async def list_threads(user: CurrentUser, access_token: str) -> list[ThreadSummary]:
    ...
    return [ThreadSummary(**row) for row in response.data]
...
async def create_thread(
    ...
    return ThreadCreated(**response.data[0])
...
async def require_thread_access(thread_id: uuid.UUID, user: CurrentUser) -> ThreadRow:
    ...
    return ThreadRow(id=uuid.UUID(str(row["id"])), user_id=owner_id, title=row["title"])


async def delete_thread(thread_id: uuid.UUID, access_token: str) -> None:
    client = await create_user_client(access_token)
    await client.table("chat_threads").delete().eq("id", str(thread_id)).execute()


def _citation_rows_from_message(assistant_message: UIMessage) -> list[dict[str, Any]]:
...
async def load_messages(thread_id: uuid.UUID, access_token: str) -> list[UIMessage]:
    client = await create_user_client(access_token)
    response = await (
